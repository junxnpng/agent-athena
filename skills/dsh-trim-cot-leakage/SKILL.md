...
1. Scope and exclusions: 명시적 스코프를 요구한다; `vendor/`·아카이브는 건드리지 않는다. Recorded fixtures and snapshots are derivatives, not prose targets: change the owning source or scenario and regenerate them only when an authorized behavior change requires new evidence.
...
3. Fix owner-first per surface: generated artifacts → fix the source (doc comments or generator template) and regenerate all derivatives; paired or translated documents → update the counterpart minimally and re-record it the way the repo defines; model- or user-visible strings → change only with owning behavior evidence, otherwise leave unchanged and report the deferral.
...
5. Verify: re-run the batteries expecting only sanctioned keeps, this skill's own directory, and the owning note's quoted evidence; confirm every remaining citation resolves at HEAD; run whatever documentation gates the repo defines for the touched surfaces.
