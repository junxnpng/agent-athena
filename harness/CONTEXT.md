...
- **2026-08-27 · 훅 주입 경로** — 러너는 `claude --plugin-dir <harness>`로 훅을 세션 한정 주입한다 (설치 불필요, 헤드리스 `-p`에서도 SessionStart/PreToolUse 동작 실측). 플러그인을 전역 설치한 채 러너를 돌리면 훅이 두 번 뜨므로 하지 않는다. 대화형: `claude --plugin-dir ~/workspace/agent-athena/harness`.
