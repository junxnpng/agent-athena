...
- **2026-08-30 · human_scope 자료는 밤이 반입 커밋한다** — 06:30 수집기(스케줄러)가 `inbox/` 에 남긴 미추적 파일 때문에 밤 preflight 가 "트리가 clean 이 아니다" 로 거부한다(예약 밤이 영원히 안 뜬다). 미추적인 채 밤이 시작됐더라도 범위 판정(`changed_paths` 는 기준선이 없다)이 위반으로 보고 실패 되돌리기(`git clean -fd`)가 수집물을 지운다. 해소: dirt 가 human_scope 안뿐이면 러너가 `[harness] human_scope 반입` 으로 커밋하고 시작(`human_intake` 이벤트), 그 밖의 dirt 가 섞이면 전부 거부. 밤을 달력에 묶는 것(일요일 23:00 → 월요일 07:00 메일)은 launchd 템플릿으로 하네스 밖에 둔다. findings/009.
- **2026-08-31 · 세션 Python 은 트리에 바이트코드를 쓰지 않는다** — macOS 시스템 Python 의 .pyc 캐시(`$HOME/Library/Caches/com.apple.python/…`)가 HOME 을 트리 안으로 둔 세션에서 repo 에 떨어져 범위 위반으로 시도 하나를 태웠다(research night-002). 러너 env 에 `PYTHONDONTWRITEBYTECODE=1`·`PYTHONPYCACHEPREFIX=sessions/pycache`. findings/010.
- **2026-08-31 · 라운드 2 리뷰 반영(보안 3 + 정확성 1)** — .harness 조작이 되돌려지지 않고 커밋돼 다음 세션이 실행하던 것(findings/011, revert_worktree 가 .harness 도 복원) · human_scope 반입이 기점 확정 전이라 다음 밤이 분기로 거부되던 것(findings/012, intake 를 checkout 뒤로) · 모델 세션 env 시크릿 제외(findings/013) · night-detached 심볼릭 로그 거부 · run-hook 이 python 없을 때 pre-tool fail-closed · plugin-refresh install 실패 시 '훅 없음' 경고. 미반영(위험 낮음): day_cost 자정 경계(밤이 자정을 넘으면 상한이 약해짐 — max_night_usd 로 하한) · lock 경합(반입이 lock 전).
- **2026-08-31 · 예산의 단위는 $ 가 아니라 창이다** — `/usage` 실측(세션 9%·주간 5%)으로 드러났다: 구독제라 $ 는 나가지 않고 `cost_usd` 는 API 정가 환산 계기판이다. `max_day_usd 45` 가 그날 두 번(밤 종료·패널 리뷰 밤 거부) 실재하지 않는 벽으로 작업을 막았다. 해소: 8개 repo 전부 `max_night_usd`·`max_day_usd` = null(기록은 유지), 실질 예산은 `rate_limit_stop 0.85`(5시간 창 사용률). SUMMARY 이상 징후에서 창 사용률을 맨 앞으로. 폭주 백스톱이 필요하면 숫자를 다시 넣는다(2026-08-29 $68 실측이 그 사례) — 그때도 의미는 '돈' 이 아니라 '창을 태우는 반복'.
- **2026-08-31 · 통과했는데 산출물이 없다** — 멀티 주제 재구성 뒤 `write_scope` 를 갱신하지 않아 밤이 `projects/**` 에 못 썼고, 리뷰 검증기가 빈 디렉토리를 통과시켜 리프 5개가 `passed` 로 남았다(findings/014). 산출물 리프의 검증기는 **존재를 요구**한다(`lab.review --expect`), preflight 는 트리에 없는 scope 경로를 경고한다.
