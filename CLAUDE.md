...
## 레이아웃
- `runner/night` 밤 하나 실행(진입점; 무변경 시도는 verify 생략+시도 소진, 끝에 refine-lite 교훈 제안 — 채택은 사람) · `runner/night-loop` 밤 여러 개 잇기(창·예산 종료 시; 예산 단위는 5시간 창 `rate_limit_stop` — $ 상한은 기본 해제, 계기판) · `runner/queue` 선택 정책·`accept`·`approve`(승인 게이트 `verify: "approval"`) · `runner/id` ID 발급 · `runner/summary` 아침 산출물
- `runner/init` 대상 repo 골격 · `runner/verify-doc` 산출물 검증기(형식·참조·숫자 — 품질은 게이트) · `runner/decompose --check` 계획 검증 · `--propose` 리프 제안(P7-lite: 제안은 러너, 채택은 사람 또는 `plan.auto_accept`; 검증기는 지금 실패해야 채택)
