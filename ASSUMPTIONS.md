...
| 검증기는 부재를 통과시키지 않는다 | 산출물을 만드는 리프의 검증기가 빈 결과를 ok 로 보면 P6 판정이 무의미해진다 — 리프가 아무것도 안 쓰고 통과한다(findings/014: 리뷰 5개 산출물 0 으로 passed). 존재를 요구한다(`--expect`·`test -f`·verify-doc 경로). 구조 변경 시 write_scope 갱신은 preflight 경고가 상기 | 구조적 · 모델 무관 | `runner/night: preflight`, 도메인 검증기 |
| A1 무변경 시도 억제 | 실패 시도는 되돌려지므로 같은 HEAD·무변경 시도의 leaf verify 재실행은 결과가 보장된 낭비다 — 실패를 (검증기, HEAD)로 캐시해 생략하고 시도만 소진시킨다. 플레이크 verify 가 우연히 통과해도 무변경이라 커밋할 게 없으므로 억제가 옳다. 모델에게는 '트리를 바꾸지 않았다'를 명시해야 헛돎을 반복하지 않는다(A2) — prime-agent unchanged-workspace 억제 이식(2026-09-02 리뷰) | 구조적 · 모델 무관 | `runner/night: noop_fail_cache`, `drivers._history` |
| B1 refine-lite 교훈 제안 | 밤의 실패 흔적을 모델이 리뷰하면 하네스 지침(프롬프트·spec·가정)의 구멍이 보인다는 가정. 제안은 러너가 쓰고 채택은 사람(P7-lite) — 증거 필수·건수/길이/밤 절 수 상한은 코드가 강제(prime-agent /refine 의 무승인 적용·무상한 축적 반면교사). 세션은 읽기 전용(Bash·쓰기·Task·Skill 금지 — 증거는 신뢰불가 콘텐츠, I7 재검사) | Claude 5 · C | `harnesslib: lesson_evidence/parse_lessons/write_lessons`, `runner/night: propose_lessons`, `prompts/lessons.md` |

## Codex 실행 가정

- Codex 대화형 이식: CLI 0.154.0의 Claude 플러그인 호환 로더와 신뢰된 훅을 전제로 `tool_input.command` 패치 경로를 검사한다. 유효 모델 급: 도구 계약을 따르는 Codex 모델, 모델 성능과 무관한 결정론적 경로 검사. I7 재검사: MCP 차단은 공유하되 호스팅 웹 검색·일부 도구는 훅 밖이므로 비공개 무인 실행을 보장하지 않는다.
- Codex 무인 드라이버: CLI 0.154.0 이상·저장된 CLI 인증·OS 샌드박스와 주입한 로컬 훅을 전제로 한다. 유효 모델 급: Codex 도구 계약을 따르는 모델. I7 재검사: 웹·플러그인·MCP·위임을 끄고 셸 네트워크를 차단하며, 별도 프로젝트 설정/훅이 있으면 시작을 거부한다. 달러·사용률·턴 상한은 지원하지 않아 명시적 null 해제 후 시간 상한으로만 실행한다. 모델 호출 자체는 네트워크를 사용하며 관리자 정책과 OS 격리는 운영 환경에서 확인한다.
- 루프 시간 예산: 다음 밤에 남은 루프 시간과 밤 상한 중 작은 값을 전달한다. 최소 작업 시간보다 적게 남으면 새 밤을 시작하지 않는다. 검증·정리에는 별도 타임아웃이 있어 전체 프로세스가 마감 시각에 즉시 끝난다는 보장은 아니다. 구조적·모델 무관 (`runner/night-loop`, findings/016).
